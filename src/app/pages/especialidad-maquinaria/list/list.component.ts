...
import { EspecialidadMaquina } from 'src/app/models/especialidad-maquina.model';
...
export class ListEspecialidadMaquinariaComponent implements OnInit {
  ...
  especialidadesmaquinaria: EspecialidadMaquina[] = []; // Array to store machinery-speciality links

  // Inject the especialidadMaquinariaService and Router (if needed)
  constructor(private EspecialidadMaquinaService: EspecialidadMaquinaService, private router: Router) { }

  ngOnInit(): void {
    // Call the service to get the list of links
    this.EspecialidadMaquinaService.list().subscribe(data => {
  ...
  delete(id: number) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.EspecialidadMaquinaService.delete(id).subscribe({
          next: () => {
